...
const ImageUploader=()=>{
    ...
    //「画像アップロード」というテキストがクリックされた時のコールバック
    const onClickText=()=>{
    ...
    const onClickUpload=async()=>{
        if(fileRef.current != null){
            //通常はここでAPIを読んで、ファイルをサーバーへアップロードする
            //ここでは疑似的に一定時間待つ
            await sleep(UPLOAD_DELAY)

            //アップロードが成功した旨を表示るためにメッセージを書き換える  
            setMessage(`${fileRef.current.name} been successfully uploaded`)
        }}
